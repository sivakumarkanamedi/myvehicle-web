...
export default function SOSPage() {
  ...
  async function recordSOSAction(
    actionType: string,
    actionLabel: string,
    actionData: Record<string, unknown> = {}
  ) {
    if (!activeSosEventId) return;
    ...
    try {
      ...
      await supabase.from("sos_event_actions").insert({
        sos_event_id: activeSosEventId,
        user_id: user.id,
      ...
      console.error("Unable to record SOS action:", error);
    }
  }
            ...
            <div className="flex flex-wrap gap-3">
              {primaryEmergencyContact ? (
                <a
                  ...
                  onClick={() =>
                    void recordSOSAction("call_contact", "Called Emergency Contact", {
                      contact_name: primaryEmergencyContact.contact_name,
                      mobile_number: primaryEmergencyContact.mobile_number,
                    })
                  }
            ...
            <div className="flex flex-wrap gap-3">
              {mapsUrl ? (
                <a
                  ...
                  rel="noreferrer"
                  onClick={() =>
                    void recordSOSAction("open_maps", "Opened SOS Location in Maps", {
                      latitude: location.status === "ready" ? location.latitude : null,
                      longitude: location.status === "ready" ? location.longitude : null,
                    })
                  }
            ...
            <div className="flex flex-wrap gap-3">
              {whatsappUrl ? (
                <a
                  ...
                  rel="noreferrer"
                  onClick={() =>
                    void recordSOSAction("whatsapp_sos", "WhatsApp SOS Opened", {
                      contact_name: primaryEmergencyContact?.contact_name || null,
                    })
                  }
              ...
              {smsUrl ? (
                <a
                  href={smsUrl}
                  onClick={() =>
                    void recordSOSAction("sms_sos", "SMS SOS Opened", {
                      contact_name: primaryEmergencyContact?.contact_name || null,
                    })
                  }
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            {roadsideAssistanceUrl ? (
              <EmergencyLink
                ...
                onClick={() =>
                  void recordSOSAction(
                    "roadside_assistance",
                    "Roadside Assistance Opened",
                    {
                      latitude: location.status === "ready" ? location.latitude : null,
                      longitude: location.status === "ready" ? location.longitude : null,
                    }
